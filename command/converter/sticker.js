...
module.exports = {
	...
	async run({ msg, conn }, { q }) {
		...
		q = q.split("|");
		const packInfo = {
			packname: q[0] ? q[0] : db.data.setting.packname,
			author: q[1] ? q[1] : db.data.setting.author,
		};
		...
		try {
			if (isQStic) {
				...
				filename = "./temp/" + Date.now() + ".webp";
				fs.writeFileSync(filename, buffer);
				await createExif(q[0] ? q[0] : db.data.setting.packname, q[1] ? q[1] : db.data.setting.author);
				await modStick(filename, conn, msg, msg.from);
				...
				await msg.reply(`Kirim atau reply media dengan caption sticker`);
			}
